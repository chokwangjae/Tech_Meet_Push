 ...
 * @description   EdgeManager에 echo를 보낸다.
 * @param         {Function} callback - 콜백함수
 * @param         {String} callback.statusCode - 코드
 * @param         {String} callback.msg - 메시지
 * @param         {Object} callback.data - 데이터
 * @param         {Object} options - object 보낼 데이터
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.edgeagent.userLogIn(callback,options);
 */
...
/**
 * @function      $h.edgeagent.getMyNewNotification
 ...
 * var callback = function(result){ console.log(result); };
 * var params = {};
 * $h.edgeagent.getMyNewNotification(callback,params);
 */
$h.edgeagent.getMyNewNotification = function (callback, options) {
    try {
        matrixMobile.plugin.edgeagent.getMyNewNotification(callback, options);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.edgeagent.getMyNotification
 * @description   나의 공지를 조회한다.
 * @param         {Function} callback - 콜백함수<br/>콜백 파라미터
 * @param         {String} callback.statusCode - 코드
 * @param         {String} callback.msg - 메시지
 * @param         {Object} callback.data - 데이터
 ...
 * var callback = function(result){ console.log(result); };
 * var params = {};
 * $h.edgeagent.getMyNotification(callback,params);
 */
$h.edgeagent.getMyNotification = function (callback, options) {
    try {
        matrixMobile.plugin.edgeagent.getMyNotification(callback, options);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.edgeagent.setMyNotificationRead
 ...
 * $h.edgeagent.setMyNotificationRead(callback,params);
 */
$h.edgeagent.setMyNotificationRead = function (callback, options) {
    try {
        matrixMobile.plugin.edgeagent.setMyNotificationRead(callback, options);
    } catch (e) {
        console.log(e);
    }
}
 ...
 * @description   전체 공지를 조회한다.
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.edgeagent.getAllNotification(callback);
 */
$h.edgeagent.getAllNotification = function (callback) {
    matrixMobile.plugin.edgeagent.getAllNotification(callback);
}
...
/**
 * @function      $h.edgeagent.sendDeviceCommand
 * @description   특정 Device에 명령을 내린다.
 * @param         {Function} callback - 콜백함수<br/>콜백 파라미터
 * statusCode - {String} 1000
 * msg - {String} Success
 * data - {Object} null
 * @param         {Object} options - object
 *  * data : 전달할 데이터 (Json객체)
 *  * receivers : 수신자들 (배열) - Device 리스트 조회시 전달받은 EdgdId
 * @example
 * var callback = function(result){ console.log(result); };
 * var params = {"data" : {"test" : "test123"}, "receivers" : ["edgeID1", "edgeID2"]} ;
 * $h.edgeagent.sendDeviceCommand(callback,params);
 */
$h.edgeagent.sendDeviceCommand = function (callback, options) {
    try {
        matrixMobile.plugin.edgeagent.sendDeviceCommand(callback, options);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.edgeagent.deviceEventListener
 * @description   Device에서 Event가 전달되었을때 Web에 전달되는 Event Listener를 등록한다.
 * @param         {Function} listener - 이벤트발생시 실행할 함수
 * @example
 * var listener = function(e){ console.log(e); };
 * $h.edgeagent.deviceEventListener(listener);
 */
$h.edgeagent.deviceEventListener = function (listener) {
    try {
        document.addEventListener('edgeAgentDeviceEventListener', listener, false);
        matrixMobile.plugin.edgeagent.deviceEventListener(function(r){});
    } catch (e) {
        console.log(e);
    }
}

// event      $h.edgeagent.onDeviceEventFire
// description   DeviceEvent가 발생했을떄 호출된다. (App에서 호출)
$h.edgeagent.onDeviceEventFire = function (data) {
    let event = new Event('edgeAgentDeviceEventListener');
    event.data = data
    document.dispatchEvent(event);
}

/**
 * @function      $h.edgeagent.removeDeviceEventListener
 * @description   Device에서 Event가 전달되었을때 Web에 전달되는 Event Listener를 제거한다.
 * @param         {Function} listener - 제거할 이벤트 리스너
 * @example
 * var listener = function(e){ console.log(e); };
 * $h.edgeagent.removeDeviceEventListener(listener);
 */
$h.edgeagent.removeDeviceEventListener = function (listener) {
    try {
        document.removeEventListener('edgeAgentDeviceEventListener', listener, false);
    } catch (e) {
        console.log(e);
    }
}
